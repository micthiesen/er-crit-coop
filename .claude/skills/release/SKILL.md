...
1. **Preflight.**
   - Ensure `gh` is installed and authed: `gh auth status`. If `gh` is missing, install it
     (`~/.confirm-sudo.sh pacman -S --needed github-cli`).
